...
if(isset($_POST['upload_internship']))
{
	include_once "dbh.inc.php";
	$email = $_SESSION['u_email'];
	$files = $_FILES['internship']; // global variable for files
	...
	$description = array('Internship_one','Internship_two','Olympiad_one','Olympiad_two');
	for($i=0;$i<$n;$i++)
	{
			...
			if(in_array($file_extension,$allowed))
			{
				if($file_error === 0)
				{
					if($file_size[$i]< 2097152)
					{
						...
						$file_des = "../img/internship/".$file_new_name;
						if(move_uploaded_file($file_tmp,$file_des))
						{
							switch($i)
							{
								case '0':
									$sql = "UPDATE internship_docs SET Internship_one = '$file_new_name' WHERE Email = '$email'";
									...
									break;
								case '1':
									$sql = "UPDATE internship_docs SET Internship_two = '$file_new_name' WHERE Email = '$email'";
									$res = mysqli_query($conn,$sql);
									if($res>0)
									{
										// nothing
									...
									break;
								case '2':
									$sql = "UPDATE internship_docs SET Olympiad_one = '$file_new_name' WHERE Email = '$email'";
									$res = mysqli_query($conn,$sql);
									if($res>0)
									{
										// nothing
									...
									break;
								case '3':
									$sql = "UPDATE internship_docs SET Olympiad_two = '$file_new_name' WHERE Email = '$email'";
									$res = mysqli_query($conn,$sql);
									if($res>0)
									{
										// nothing
