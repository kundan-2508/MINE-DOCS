...
if(isset($_POST['upload_higher_secondary']))
{
	include_once "dbh.inc.php";
	$email = $_SESSION['u_email'];
	$files = $_FILES['higher_secondary']; // global variable for files
			...
			if(in_array($file_extension,$allowed))
			{
				if($file_error === 0)
				{
					if($file_size[$i]< 2097152)
					{
						...
						$file_des = "../img/higher_secondary/".$file_new_name;
						if(move_uploaded_file($file_tmp,$file_des))
						{
							switch($i)
							{
								case '0':
									$sql = "UPDATE higher_secondary_docs SET Marksheet = '$file_new_name' WHERE Email = '$email'";
									...
									break;
								case '1':
									$sql = "UPDATE higher_secondary_docs SET AdmitCard = '$file_new_name' WHERE Email = '$email'";
									$res = mysqli_query($conn,$sql);
									if($res>0)
									{
										// nothing
									...
									break;
								case '2':
									$sql = "UPDATE higher_secondary_docs SET Others = '$file_new_name' WHERE Email = '$email'";
									$res = mysqli_query($conn,$sql);
									if($res>0)
									{
										// nothing
